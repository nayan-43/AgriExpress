{{--
  ...
  Populated by App\Http\Controllers\Admin\CategoryController@index.
--}}
...
<div class="flex flex-wrap gap-3 items-center justify-between mb-5">
    <div><h1 class="text-2xl font-bold">Categories</h1><p class="muted text-sm mt-1">Manage product categories</p></div>
    <a href="{{ route('admin.categories.create') }}" class="bg-blue-600 hover:bg-blue-500 text-white rounded-lg px-4 h-10 text-sm font-medium flex items-center"><i class="fa-solid fa-plus mr-2"></i>Add category</a>
</div>
...
<div class="surface border rounded-2xl scroll-x">
    <table class="w-full text-sm min-w-[760px]">
        <thead><tr class="text-left muted text-[12.5px]" style="background:var(--bg)">
            ...
            <th class="px-4 py-3 font-medium">Parent</th><th class="px-4 py-3 font-medium">Slug</th>
        ...
        <tbody class="divide-b">
            @forelse ($categories as $c)
                <tr>
                    ...
                    <td class="px-4 py-3">
                        <x-action-buttons
                            :edit-route="route('admin.categories.edit', $c)"
                            :delete-route="route('admin.categories.destroy', $c)" />
                    </td>
                </tr>
            @empty
                <x-empty-state :colspan="8" message="No categories yet. Create one to start organizing products." />
            @endforelse
        </tbody>
    </table>
