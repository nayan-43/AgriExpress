@if (!isset($hideNewsletter))
    <section class="bg-brand-800 text-white">
        <div class="max-w-7xl mx-auto px-4 py-8 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <form action="{{ url('/newsletter') }}" method="POST" class="flex w-full md:w-auto gap-2">
                @csrf
                <input type="email" name="email" required placeholder="Enter your email address"
                    class="rounded-md px-4 py-2 text-gray-800 w-full md:w-72 outline-none">
                <button
        ...
        <div class="max-w-7xl mx-auto px-4 py-12 grid sm:grid-cols-2 md:grid-cols-5 gap-8">
            <div>
                <div class="flex items-center gap-2 mb-3">
                    <span class="w-8 h-8 rounded-lg bg-brand-700 text-white flex items-center justify-center"><i
                            class="fa-solid fa-bag-shopping text-sm"></i></span>
