...
--}}
<header class="sticky top-0 z-20 h-17 surface border-b flex items-center gap-4 px-4 sm:px-6">
    <button class="text-slate-500" onclick="toggleSidebar()" aria-label="Open menu">
        <i class="fa-solid fa-bars"></i>
    </button>

    {{-- <form action="{{ route('admin.search') ?? '#' }}" method="GET" class="hidden sm:flex items-center gap-2 flex-1 max-w-107.5 rounded-xl px-4 h-10 border bd" style="background:var(--bg)">
