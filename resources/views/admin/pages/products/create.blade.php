        ...
        <div class="flex flex-wrap gap-3 items-center justify-between mb-5">
            <div>
                ...
                <p class="muted text-sm mt-1">
                    {{ $product->exists ? 'Update this product\'s details' : 'Create a new product for your store' }}</p>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('admin.products.index') }}"
                    class="surface border rounded-lg px-4 h-10 text-sm flex items-center">Cancel</a>
                <button type="submit" name="status" value="0"
                    class="surface border rounded-lg px-4 h-10 text-sm">Save draft</button>
                <button type="submit" name="status" value="1"
                    class="bg-blue-600 hover:bg-blue-500 text-white rounded-lg px-4 h-10 text-sm font-medium"><i
                        class="fa-solid fa-paper-plane mr-2"></i>Publish</button>
            </div>
        </div>
                ...
                <div class="surface border rounded-2xl p-5">
                    ...
                    <div class="grid sm:grid-cols-2 gap-4 text-sm">
                        <div class="sm:col-span-2">
                            <label for="name" class="block mb-1.5 font-medium">Product name <span
                        ...
                        <div>
                            <label for="sku" class="block mb-1.5 font-medium">SKU <span
                                    class="text-red-500">*</span></label>
                            <input id="sku" name="sku" required value="{{ old('sku', $product->sku) }}"
                                class="w-full border bd rounded-lg px-3 h-10" placeholder="Generated from product name"
                                {{ $product->exists ? '' : 'readonly' }}>
                            @if (!$product->exists)
                                <p class="muted text-[11.5px] mt-1">Generated automatically from the product name.</p>
                            @endif
                        </div>
                        <div>
                            <label for="category_id" class="block mb-1.5 font-medium">Category <span
                                    class="text-red-500">*</span></label>
                        ...
                        <div>
                            ...
                            <select id="brand_id" name="brand_id" class="w-full border bd rounded-lg px-3 h-10">
                                ...
                                @foreach ($brands as $b)
                                    <option value="{{ $b->id }}" @selected(old('brand_id', $product->brand_id) == $b->id)>{{ $b->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="sm:col-span-2">
                            ...
                            <textarea id="short_description" name="short_description" rows="2" class="w-full border bd rounded-lg px-3 py-2"
                ...
                <div class="surface border rounded-2xl p-5">
                    ...
                    <div class="border bd rounded-lg">
                        <div class="flex flex-wrap gap-1 p-2 border-b bd">
                            <button type="button" class="w-8 h-8 rounded hover:bg-black/5"><i
                                    class="fa-solid fa-bold text-xs"></i></button>
                            <button type="button" class="w-8 h-8 rounded hover:bg-black/5"><i
                                    class="fa-solid fa-italic text-xs"></i></button>
                            <button type="button" class="w-8 h-8 rounded hover:bg-black/5"><i
                                    class="fa-solid fa-underline text-xs"></i></button>
                            <span class="w-px h-6 self-center mx-1" style="background:var(--border)"></span>
                            <button type="button" class="w-8 h-8 rounded hover:bg-black/5"><i
                                    class="fa-solid fa-list-ul text-xs"></i></button>
                            <button type="button" class="w-8 h-8 rounded hover:bg-black/5"><i
                                    class="fa-solid fa-list-ol text-xs"></i></button>
                            <span class="w-px h-6 self-center mx-1" style="background:var(--border)"></span>
                            <button type="button" class="w-8 h-8 rounded hover:bg-black/5"><i
                                    class="fa-solid fa-align-left text-xs"></i></button>
                            <button type="button" class="w-8 h-8 rounded hover:bg-black/5"><i
                                    class="fa-solid fa-link text-xs"></i></button>
                        </div>
                        <div id="descriptionEditor" contenteditable class="min-h-40 p-3 text-sm">{!! old('description', $product->description) !!}
                        </div>
                    ...
                    <div class="grid sm:grid-cols-2 gap-4 text-sm">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea id="meta_description" name="meta_description" rows="2"
                                class="w-full border bd rounded-lg px-3 py-2">{{ old('meta_description', $product->meta_description) }}</textarea>
                        </div>
                        <div class="sm:col-span-2">
                            ...
                            <input id="meta_keywords" name="meta_keywords"
                                ...
                                class="w-full border bd rounded-lg px-3 h-10" placeholder="comma, separated, keywords">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-5">
                ...
                <div class="surface border rounded-2xl p-5">
                    ...
                    <label for="main_image"
                        class="block relative border-2 border-dashed rounded-xl text-center cursor-pointer overflow-hidden"
                        style="border-color:#c7d6f5;background:var(--bg)">
                        <div id="mainImagePreviewWrap" class="{{ $product->main_image_url ? '' : 'hidden' }}">
                            <img id="mainImagePreview" src="{{ $product->main_image_url }}" alt=""
                        ...
                        <div id="mainImagePlaceholder" class="py-8 {{ $product->main_image_url ? 'hidden' : '' }}">
                            ...
                            <p class="text-[11.5px] muted mt-1">JPG, PNG, WEBP &mdash; resized to max 1600px wide</p>
                        </div>
                        <input type="file" id="main_image" name="main_image" accept="image/*" class="hidden">
                    </label>
                    <p class="text-[11.5px] muted mt-2">Large product image, resized proportionally to a maximum width of
                        1600px.</p>
                </div>
                ...
                <div class="surface border rounded-2xl p-5">
                    ...
                    <p class="text-[11.5px] muted mb-3">Extra photos shown on the product page (angles, packaging, in
                        use...).</p>

                    <label for="gallery" class="block border-2 border-dashed rounded-xl py-6 text-center cursor-pointer"
                        style="border-color:#c7d6f5;background:var(--bg)">
                        <i class="fa-solid fa-images text-xl text-blue-500"></i>
                        <p class="text-sm mt-2 font-medium">Add gallery images</p>
                        <p class="text-[11.5px] muted mt-1">You can select multiple files; thumbnails are resized to 800px
                            wide</p>
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            @foreach ($product->images as $image)
                                <label class="relative block rounded-lg overflow-hidden border bd group">
                                    ...
                                    <span
                                        class="absolute inset-0 bg-black/50 opacity-0 group-has-checked:opacity-100 flex items-center justify-center transition-opacity">
                ...
                <div class="surface border rounded-2xl p-5">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <label for="price" class="block mb-1.5 font-medium">Regular price <span
                                    class="text-red-500">*</span></label>
                        ...
                        <div class="col-span-2">
                            <label for="stock" class="block mb-1.5 font-medium">Stock quantity <span
                                    class="text-red-500">*</span></label>
                    ...
                    <label class="flex items-center gap-2 mt-4 text-sm cursor-pointer">
                        <input type="checkbox" name="featured" value="1" class="w-4 h-4 rounded accent-blue-600"
...
@push('scripts')
    <script>
        ...
        function syncDescription() {
            descInput.value = editor.innerHTML;
        }
        editor.addEventListener('input', syncDescription);
        syncDescription();

        // Generate a readable SKU from the product name while creating a product.
        const productNameInput = document.getElementById('name');
        const skuInput = document.getElementById('sku');
        if (productNameInput && skuInput && !skuInput.readOnly) {
            const generateSku = () => {
