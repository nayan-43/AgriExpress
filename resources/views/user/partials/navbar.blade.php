<header class="border-b border-gray-100 sticky top-0 bg-white z-40">
    <div class="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between gap-6">
        ...
        <a href="{{ route('index') }}" class="flex items-center gap-2 shrink-0">
            ...
            <img src="{{ asset('assets/images/logo.png') }}" title="AgriExpress" alt="AgriExpress"
        ...
        <nav class="hidden lg:flex items-center gap-8 text-sm font-medium text-gray-700">
            <a href="{{ route('index') }}"
                class="{{ request()->routeIs('index') ? 'text-brand-700 font-semibold border-b-2 border-brand-700 pb-1' : 'hover:text-brand-700' }}">Home</a>

            <div class="relative" data-dropdown>
                <button data-dropdown-trigger
                    class="flex items-center gap-1 {{ request()->routeIs('shop', 'product') ? 'text-brand-700 font-semibold' : 'hover:text-brand-700' }}">
            ...
            <div class="relative" data-dropdown>
                ...
                <div data-dropdown-menu
                    class="hidden absolute top-full left-0 mt-3 w-56 bg-white border border-gray-100 rounded-lg shadow-lg py-2 z-50 grid-cols-1">
            ...
            <a href="{{ route('about') }}"
                class="{{ request()->routeIs('about') ? 'text-brand-700 font-semibold border-b-2 border-brand-700 pb-1' : 'hover:text-brand-700' }}">About</a>
            {{-- <a href="{{ route('blog') }}" class="hover:text-brand-700">Blog</a> --}}
            <a href="{{ route('contact') }}"
                class="{{ request()->routeIs('contact') ? 'text-brand-700 font-semibold border-b-2 border-brand-700 pb-1' : 'hover:text-brand-700' }}">Contact</a>
        </nav>
        ...
        <div class="flex items-center gap-4">
            ...
            <a href="{{ auth()->check() ? route('account') : route('login') }}"
                class="{{ request()->routeIs('account*') ? 'text-brand-700' : 'text-gray-700 hover:text-brand-700' }}">
                <i class="fa-regular fa-user text-lg"></i>
            </a>
            <a href="{{ auth('web')->check() ? route('wishlist') : route('login') }}"
                class="{{ request()->routeIs('wishlist') ? 'text-brand-700' : 'text-gray-700 hover:text-brand-700' }}"><i
                    class="fa-regular fa-heart text-lg"></i></a>
            <a href="{{ route('cart') }}"
                class="relative {{ request()->routeIs('cart*', 'checkout*') ? 'text-brand-700' : 'text-gray-700 hover:text-brand-700' }}">
                <i class="fa-solid fa-cart-shopping text-lg"></i>
                <span data-cart-badge
                    class="absolute -top-2 -right-2 bg-brand-700 text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center">{{ auth('web')->user()?->cart?->items()->sum('quantity') ?? 0 }}</span>
            </a>
    ...
    <div data-mobile-menu-panel
        ...
        <a href="{{ route('index') }}"
            class="block py-1 {{ request()->routeIs('index') ? 'text-brand-700 font-semibold' : '' }}">Home</a>
        <a href="{{ route('shop') }}"
            class="block py-1 {{ request()->routeIs('shop', 'product') ? 'text-brand-700 font-semibold' : '' }}">Shop</a>
        <a href="{{ route('shop') }}"
            class="block py-1 {{ request()->routeIs('shop', 'product') ? 'text-brand-700 font-semibold' : '' }}">Categories</a>
        <a href="{{ route('about') }}"
            class="block py-1 {{ request()->routeIs('about') ? 'text-brand-700 font-semibold' : '' }}">About</a>
        {{-- <a href="{{ route('blog') }}" class="block py-1">Blog</a> --}}
        <a href="{{ route('contact') }}"
            class="block py-1 {{ request()->routeIs('contact') ? 'text-brand-700 font-semibold' : '' }}">Contact</a>
    </div>
</header>
